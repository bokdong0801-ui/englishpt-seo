...
GUIDE_OBSERVE=[
 "최근 수행에서 혼자 처리한 범위를 먼저 표시하고",
 "가장 가까운 일정에서 실제로 해야 할 행동을 하나 고르고",
 "지난 연습에서 반복해서 멈춘 조건을 짧게 적고",
 "익숙한 문제에서 되는 부분과 새 조건에서 흔들리는 부분을 나누고",
 "설명을 듣기 전 첫 시도에서 어떤 판단을 했는지 확인하고",
 "도움을 받았을 때 바로 수정된 부분과 끝까지 남은 부분을 구분하고",
 "현재 자료에서 시간 사용이 길어진 구간을 먼저 찾아보고",
 "최근 두 번의 결과에서 같은 실수가 겹치는 지점을 비교하고",
 "이미 안정된 기능은 유지 항목으로 빼고 남은 병목을 좁히고",
 "처음부터 전체 수준을 다시 보지 않고 가장 불안정한 행동을 고르고",
 "실제 질문이나 자료 앞에서 첫 반응이 늦어진 순간을 기록하고",
 "정답 여부보다 왜 그런 선택이나 응답을 했는지를 먼저 설명해보고",
 "다음 사용 장면에 꼭 필요한 기능과 있으면 좋은 기능을 분리하고",
 "복습할 수 있는 실제 시간 안에서 다시 볼 범위를 줄이고",
 "한 번 성공한 결과가 어떤 도움에 의존했는지 확인하고",
 "상담 전에 최근 자료 한 개와 가장 어려웠던 장면 하나를 정리하고"
]
GUIDE_RECHECK=[
 "비슷하지만 다른 자료에서도 같은 기준이 남는지 다시 확인합니다.",
 "준비 시간을 줄였을 때도 같은 행동을 혼자 이어가는지 비교합니다.",
 "힌트를 한 단계 줄여도 스스로 수정되는 범위가 유지되는지 봅니다.",
 "질문 표현을 바꾼 뒤에도 핵심을 다시 구성할 수 있는지 살핍니다.",
 "실전과 비슷한 시간 안에서 시작과 마무리가 유지되는지 기록합니다.",
 "다음 날 다시 꺼냈을 때 기억과 적용이 함께 남아 있는지 확인합니다.",
 "새 문제에서 같은 오류가 반복되는지 보고 복습 순서를 조정합니다.",
 "자료의 난도를 크게 올리지 않고 조건만 바꿔 재사용 범위를 봅니다.",
 "수업 밖에서 다시 시도한 결과를 가져와 다음 우선순위를 정합니다.",
 "도움 없이 처리되는 항목은 제외하고 끝까지 남은 문제만 이어갑니다.",
 "다른 예시에서도 근거를 설명할 수 있는지 확인해 암기와 적용을 구분합니다.",
 "실수 원인이 달라지면 같은 과제를 반복하지 않고 연습 방식을 바꿉니다.",
 "가까운 일정이 끝난 뒤 유지할 기초 항목을 별도로 남깁니다.",
 "학습량을 더하기 전에 현재 배분이 실제 결과와 맞는지 다시 봅니다.",
 "다음 수업에서는 이전 기록에서 바로 시작해 같은 설명의 반복을 줄입니다.",
 "새로운 목표가 생겨도 이미 되는 부분은 유지하고 필요한 기능만 추가합니다."
]
def guide(row,slot):
 n=seed_int(row,slot)+slot*911
 a=GUIDE_OBSERVE[(n + slot*3)%len(GUIDE_OBSERVE)]
 b=GUIDE_RECHECK[((n//7) + slot*5)%len(GUIDE_RECHECK)]
 return a+" "+b
...
def variation_story(row,d,service):
 ...
 titles=["현재를 보는 관점","수업을 고르는 관점","지역과 생활 맥락","연습 전후 비교","설명 방식","진단 초점","정보를 배열하는 순서","선택을 좁히는 순서","상담으로 연결하는 기준","독립 검토 관점 A","독립 검토 관점 B"]
