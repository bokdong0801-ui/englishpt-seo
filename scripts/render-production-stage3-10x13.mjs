...
for(const item of manifest.files){
 ...
 for(const vp of viewports){
  ...
  const metrics=navError?null:await page.evaluate(()=>{
   ...
   return {h1Count:document.querySelectorAll('h1').length,h1:h1?.textContent?.trim()||'',h1Visible:vis(h1),scrollWidth:document.documentElement.scrollWidth,bodyScrollWidth:document.body.scrollWidth,innerWidth:innerWidth,phoneVisible:vis(phone),detailVisible:vis(detail),consultVisible:vis(consult),decisionStrip:!!document.querySelector('.decision-strip'),decisionGuide:!!document.querySelector('.decision-guide'),midCta:!!document.querySelector('.mid-cta'),robots:document.querySelector('meta[name="robots"]')?.content||'',canonical:document.querySelector('link[rel="canonical"]')?.href||'',bodyFlag:document.body?.dataset?.productionDeploy||'',bodyHeight:document.body?.scrollHeight||0};
  });
  const overflow=metrics?Math.max(metrics.scrollWidth,metrics.bodyScrollWidth)>metrics.innerWidth+1:true;
  const pass=!navError&&errors.length===0&&!overflow&&metrics.h1Count===1&&metrics.h1===item.h1&&metrics.h1Visible&&metrics.phoneVisible&&metrics.detailVisible&&metrics.consultVisible&&metrics.decisionStrip&&metrics.decisionGuide&&metrics.midCta&&metrics.robots.toLowerCase().includes('noindex')&&metrics.canonical===item.canonical&&metrics.bodyFlag==='false'&&metrics.bodyHeight>vp.height;
  if(!pass)failures++;
