...
VAR={
 ...
 "error":{
  "intro":"같은 막힘이나 불완전한 응답도 원인이 다를 수 있습니다. 필요한 지식이 부족했는지, 질문을 다르게 이해했는지, 시간 때문에 놓쳤는지, 알고도 바로 적용하지 못했는지를 분리해야 연습 방식이 달라집니다.",
...
VAR_GUIDE_LAST={
 ...
 "deadline":[
  "계획을 세운 뒤에는 매주 목표를 새로 만드는 대신 남은 일정표에서 완료된 항목을 지우고 미완료 항목의 우선순위만 다시 조정합니다. 이렇게 하면 시험이 가까워질수록 계획이 복잡해지는 것을 막고, 지금 해야 할 행동과 시험 후로 넘길 행동을 계속 구분할 수 있습니다.",
  "시험일이 다가올수록 하루 계획도 단순하게 유지합니다. 그날 꼭 확인할 병목 하나, 실전 조건으로 재검사할 항목 하나 정도를 정하고 완료 여부를 기록하면 촉박한 시기에 공부 범위가 다시 넓어지는 것을 막을 수 있습니다."
 ],
 "error":[
...
def scene_blocks(exam,var):
 ...
 for i,(title,symptom) in enumerate(exam["scenes"]):
  ...
  if var=="scene":
   ...
   extra=[f"다음 시험에서 '{title}' 항목이 중요하다면 남은 연습 횟수 안에서 직접 바꿀 수 있는 행동부터 정합니다.",
...
def feedbacks(exam,var):
 ...
 if var=="scene":
  return [
   f"'{a}' 항목은 익숙한 조건에서는 처리했지만 문항이나 자료가 달라지면 반응이 늦어짐. 다음에는 같은 판단 기준을 새 조건에 적용해보기.",
   f"'{b}' 항목은 설명을 들은 뒤에는 가능하지만 혼자 다시 시도하면 같은 막힘이 나타남. 다음에는 힌트를 줄이고 스스로 기준을 먼저 말해보기.",
   "시험일 전에는 새 범위를 늘리기보다 최근 두 번의 수행에서 반복된 병목만 다시 확인하기."
  ]
 if var=="deadline":
  return [
   f"시험일까지 남은 기간을 고려해 '{a}'의 새 범위를 늘리기보다 현재 가장 흔들리는 행동을 먼저 안정시키기로 함. 다음 실전 연습에서 같은 기준이 유지되는지 확인.",
   f"'{b}' 항목은 내용 자체보다 제한된 시간 안에서 처리하는 과정이 더 불안정함. 다음에는 실제 시간 조건을 적용하고 시작과 종료 지점을 기록해보기.",
   "마감 직전에는 학습량보다 실전 조건에서 반복해서 흔들리는 두 항목만 재점검하기."
  ]
 if var=="error":
  return [
   f"'{a}' 항목은 첫 시도에서 기준이 흔들렸지만 짧은 힌트 뒤에는 수정됨. 다음에는 비슷한 새 문제나 응답에서 힌트 없이 다시 확인하기.",
   f"'{b}' 항목은 설명 직후에는 가능하지만 조건이 달라지면 다시 흔들림. 다음에는 어떤 단서가 필요한지 먼저 기록하고 도움을 한 단계 줄여보기.",
   "같은 막힘이 새 문제나 새 질문에서도 반복되는지 확인하고, 스스로 수정 가능한 항목은 다음 목록에서 제외하기."
  ]
 if var=="use":
  return [
   f"다음 시험에서는 '{a}' 항목을 실제 행동으로 안정시키는 것을 첫 목표로 정함. 수업에서는 직접 수행한 뒤 다음 practice에서 다시 확인.",
   f"'{b}' 항목은 설명을 듣는 것보다 직접 풀거나 답을 만들 때 더 흔들림. 다음에는 같은 기능을 다른 문제에 적용하고 결과를 기록하기.",
   "학습 항목 목록보다 다음 시험에서 실제로 해야 할 행동 두 가지를 정해 연습하기."
  ]
 return [
  f"익숙한 '{a}' 조건에서는 가능하지만 자료나 질문이 달라지면 처리 속도가 늦어짐. 다음에는 준비 시간을 줄이고 새 조건에서 재확인.",
  f"'{b}' 항목은 예시가 있을 때 안정적이지만 도움을 없애면 구조가 흔들림. 다음에는 예시 없이 핵심 기준부터 다시 구성하기.",
  "한 번의 성공보다 다른 문제나 응답에서도 같은 기준을 다시 사용할 수 있는지를 다음 평가 기준으로 두기."
 ]
