...
def feedbacks(exam,var):
 ...
 if var=="deadline":
  return [
   ...
   f"'{b}' 항목은 정확도보다 시간 사용이 더 큰 병목으로 확인됨. 다음에는 풀이 순서를 고정하고 종료 시점을 기록해보기.",
 ...
 if var=="use":
  return [
   ...
   f"'{b}' 항목은 설명을 듣는 것보다 직접 답을 만들 때 흔들림. 다음에는 같은 기준을 다른 문제에 적용하고 결과를 기록하기.",
   "학습 항목 목록보다 다음 시험에서 실제로 해야 할 행동 두 가지를 정해 연습하기."
  ]
 return [
  f"익숙한 {a} 문제에서는 가능하지만 자료가 달라지면 처리 속도가 늦어짐. 다음에는 준비 시간을 줄이고 새 문항에서 재확인.",
  f"'{b}' 항목은 예시가 있을 때는 안정적이지만 도움을 없애면 구조가 흔들림. 다음에는 예시 없이 핵심 기준부터 다시 구성하기.",
